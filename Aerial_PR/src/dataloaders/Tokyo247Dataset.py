...
class Tokyo247Dataset(Dataset):
    """
    Tokyo247 evaluation dataset for visual place recognition.

    Expected files in `dataset_path`:
      - tokyo247_dbImages.npy
      - tokyo247_qImages.npy
      - tokyo247_gt.npy
    ...
    def __init__(
        ...
        self.dbImages = np.load(self.dataset_path / "tokyo247_dbImages.npy")
        self.qImages = np.load(self.dataset_path / "tokyo247_qImages.npy")
        self.ground_truth = np.load(self.dataset_path / "tokyo247_gt.npy", allow_pickle=True)
    ...
    def _validate_path(self, dataset_path):
        if dataset_path is None:
            dataset_path = Path(__file__).parent.parent.parent / "data" / "tokyo247"
        ...
        required_files = ["tokyo247_dbImages.npy", "tokyo247_qImages.npy", "tokyo247_gt.npy"]
        missing = [f for f in required_files if not (path / f).is_file()]
        if missing:
            raise FileNotFoundError(
                f"Missing required Tokyo247 metadata files in {dataset_path}: {missing}"
            )
        return path
